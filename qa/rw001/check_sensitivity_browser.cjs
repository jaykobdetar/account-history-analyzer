#!/usr/bin/env node
// RW-001 QA derivative of scripts/check_report_browser.cjs.
// Only adds overview DOM observations and opens native candidate details for a screenshot.
// Original runtime script/report files are unchanged; all existing safety checks remain.
...
(async () => {
  ...
  try {
    ...
    const evaluated = await call('Runtime.evaluate', {returnByValue: true, expression: `(() => ({
      ...
      overviewRows: Array.from(document.querySelectorAll('table')).filter(t=>t.querySelector('th')?.innerText==='Stream' && t.querySelectorAll('th')[1]?.innerText==='Setting λ').flatMap(t=>Array.from(t.querySelectorAll('tbody tr')).map(r=>Array.from(r.cells).map(c=>c.innerText))),
      candidateSummaries: Array.from(document.querySelectorAll('summary')).map(s=>s.innerText).filter(s=>s.startsWith('Stored candidates:'))
    }))()`}, sessionId);
    ...
    if (process.argv[3]) {
      if (process.argv[4]) {
        const scrolled = await call('Runtime.evaluate', {returnByValue: true,
          expression: `(() => {const requested=${JSON.stringify(process.argv[4])}; const target=requested==='rw001-overview' ? Array.from(document.querySelectorAll('h4')).find(e=>e.textContent==='Penalty-setting overview') : document.getElementById(requested); if (!target) return false; if(requested==='rw001-overview') for(const d of document.querySelectorAll('details')) if(d.querySelector('summary')?.textContent.startsWith('Stored candidates:')) d.open=true; target.scrollIntoView(); return true;})()`}, sessionId);
