...
(async () => {
  ...
  try {
    ...
    await loaded;
    // QA-only browser inspection; none of this script is embedded in reports.
    const inspectedLabels = await call('Runtime.evaluate', {returnByValue: true, expression: `(() => {
      const target=Array.from(document.querySelectorAll('table')).find(table=>
        table.previousElementSibling?.textContent.startsWith('cosine_distance_v1, retained_prose, n=4:'));
      if (!target) throw new Error('Expected retained-prose n=4 table');
      let parent=target.parentElement, comparison=null;
      while(parent) {
        if(parent.tagName==='DETAILS') {
          parent.open=true;
          const summary=parent.querySelector(':scope > summary')?.textContent || '';
          if(summary.startsWith('Comparison ')) comparison=summary.slice(11).split(' — ')[0];
        }
        parent=parent.parentElement;
      }
      target.previousElementSibling.scrollIntoView();
      return {comparison,labels:Array.from(target.querySelectorAll('tbody tr')).map(row=>row.cells[0].innerText)};
    })()`}, sessionId);
    const labelAudit=inspectedLabels.result.value;
    if(!labelAudit) throw new Error(JSON.stringify(inspectedLabels));
    const saved=JSON.parse(fs.readFileSync(path.join(path.dirname(source),'results.json'),'utf8'));
    const comparison=saved.modules.style.payload.comparisons.find(row=>row.comparison_id===labelAudit.comparison);
    const measured=comparison.distances.find(row=>row.method_id==='cosine_distance_v1' && row.view==='retained_prose' && row.n===4);
    labelAudit.expected=measured.contributions.slice(0,10).map(row=>row.feature_id);
    labelAudit.decoded=labelAudit.labels.map(label=>JSON.parse(label));
    labelAudit.distinctVisible=new Set(labelAudit.labels).size;
    labelAudit.exactSourceMatch=JSON.stringify(labelAudit.decoded)===JSON.stringify(labelAudit.expected);
    if(!labelAudit.exactSourceMatch || labelAudit.distinctVisible!==10) throw new Error('Rendered labels lost identity');
    ...
    const audit = {browser: version.product, isolation: process.env.AHAS_BROWSER_NETWORK_ISOLATION || 'cdp_offline_and_fetch_interception',
      remoteRequests, dialogs, labelAudit, ...evaluated.result.value};
