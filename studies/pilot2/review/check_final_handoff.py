...
def strings(value, *, allowed_text_mapping=False):
    if isinstance(value, dict):
        ...
        if allowed_text_mapping and isinstance(value.get('text'), dict):
            blocked.discard('text')
        assert not blocked, 'Private source field in sanitized JSON'
        for item in value.values():
            yield from strings(item, allowed_text_mapping=allowed_text_mapping)
...
def safety_check(path):
    ...
    for relative, data in handoff_files(path):
        ...
        if name.suffix.lower() == '.json':
            known_config = relative == 'protocol/resolved_config.json'
            if known_config:
                assert data == (ROOT / 'protocol/resolved_config.json').read_bytes()
            try:
                visible_texts.extend(strings(json.loads(value), allowed_text_mapping=known_config))
            except AssertionError as exc:
                raise AssertionError(str(exc) + ': ' + relative) from exc
