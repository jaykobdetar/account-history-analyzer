"""Independent metadata-only recomputation of all fixed pilot4 feasibility cuts.
...
def verify(plan_path,flags_path,registration_path,census,private,receipt_path):
    ...
    require(len(plan['cuts'])==101 and len(set(plan['cuts']))==101 and len(plan['community_pairs'])==3,'fixed_grid_changed')
    ...
    for source in plan['source_metadata']:
        ...
        with Path(source['path']).open('rb') as f:
            for raw in f:
                ...
                require(total_bytes<=256*1024**2,'metadata_byte_cap')
                counts['metadata_rows']+=1
                require(counts['metadata_rows']<=1000000,'metadata_row_cap')
    ...
    require(len(table)==303 and len({(r['stratum_id'],r['cut']) for r in table})==303,'calendar_table_coverage')
    ...
    # A stratum with fewer than two maximum matching edges proves no combined
    # two-per-stratum witness can exist, independent of account overlaps.
    obvious_deficit=any(len(v['pairs'])<2 for v in best.values())
    if obvious_deficit:
        require(witness['globally_disjoint_intended_block_witness'] is None and
                summary['globally_disjoint_intended_blocks_exist'] is False and summary['status']=='fixed_design_capacity_deficit',
                'impossible_global_witness_claimed')
    else:
        # This verifier does not silently infer a missing joint witness. If
        # capacities are sufficient, validate any supplied witness explicitly.
        joint=witness['globally_disjoint_intended_block_witness']
        require(joint is not None and set(joint)==set(best),'joint_witness_missing_requires_separate_search')
        occupied=set()
        for sid,pairs in joint.items():
            require(len(pairs)==2,'joint_block_quota')
            for pair in pairs:
                require(tuple(pair) in best[sid]['edges'] and not occupied.intersection(pair),'joint_edge_or_account_reuse')
    ...
    require(len(csv_rows)==303,'csv_row_count')
    ...
    return {'status':'passed','metadata_rows':counts['metadata_rows'],'metadata_bytes':total_bytes,
            'calendar_rows_independently_recomputed':303,'chosen_candidate_cells_verified':checked_cells,
            'chosen_candidate_records_verified':len(confirmed),'all_exact_matching_objectives_and_cut_ties_verified':True,
            'maximum_blocks_by_stratum':{s:len(v['pairs']) for s,v in best.items()},
            'chosen_cuts':{s:v['row']['cut'] for s,v in best.items()},
            'global_two_per_stratum_target_feasible':not obvious_deficit,'preprocessor_calls':0,'style_scores_computed':0,
            'source_archive_reads':0,'private_identifiers_published':False}


def main():
    p=argparse.ArgumentParser()
    for n in ('plan','flags','registration','census','private','receipt','out'):
    ...
    binding={'checker_sha256':sha(__file__),'status':'bound_before_independent_metadata_read',
             'input_hashes':{n:sha(getattr(a,n)) for n in ('plan','flags','registration','private','receipt')},
             'census_hashes':{f.name:sha(f) for f in a.census.glob('*') if f.is_file()},
             'wall_seconds_limit':600,'address_space_bytes':4*1024**3,'metadata_rows_limit':1000000}
    save(a.out.with_suffix('.start-binding.json'),binding)
    try:
        result=verify(a.plan,a.flags,a.registration,a.census,a.private,a.receipt)
