"""Independent read-only binding review v2: hashes, allocation, exclusions and byte limits.

Version 2 permits evidence transitively bound through the allocation registration.
The original checker and failed attempt are retained unchanged.
...
def nested_binding_count(direct, nested):
    """Nested evidence is covered by the checked registration, not duplicated."""
    require(all(direct[path] == digest for path, digest in nested.items() if path in direct),
            'nested_binding_hash_disagrees')
    return len(set(nested) - set(direct))
...
def review(plan_path,registration_path,allocation_path,allocation_registration_path,expected_sha):
    ...
    transitive_count=nested_binding_count(bound,allocation_bindings)
    seen=set()
    for sid in STRATA:
    ...
    return {'status':'passed','plan_sha256':expected_sha,'verified_bound_artifacts':len(bound),'additional_transitively_bound_artifacts':transitive_count,
