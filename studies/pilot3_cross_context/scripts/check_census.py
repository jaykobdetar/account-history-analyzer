...
def reconcile(plan_path, census_root, private_root, receipt_path, source_root=None):
    ...
    check(binding['script_sha256'] == fingerprint(Path(__file__).with_name('census.py'))['sha256'], 'Fresh run bound to supplied census script bytes')
    ...
    return {
        ...
        'limitations': [
            ...
            'Globally disjoint allocation optimality is checked by the separate study-math review; this checker recomputes input memberships and per-pair capacity.',
