...
Each stratum has 20 accounts/10 blocks, four early anchors per block, and 160 qualified comparisons per method. An ordering is 1 when the different-account distance exceeds the same-account distance, 0 otherwise, and 1/2 for an exact tie. Equal block means retain repeated-anchor dependence. Within and cross changes use the same ten complete blocks. “Stress” compares different-account/same-community with same-account/different-community writing.
...
All 360 main and 120 replay batches exited successfully within their unchanged input limits and declared operational ceilings. Observed outer wall times were {main_run['wall_seconds']:.2f} seconds for the main run and {replay_run['wall_seconds']:.2f} seconds for replay, with at most two evaluator processes concurrently. [Per-batch resources](../results/normalized/batch-resources.json) retain actual wall time, peak memory, input/eligible words/records, and artifact bytes. Raw actual commands, exit codes, and receipts remain preserved; sanitized execution exports identify any local-path redaction explicitly. Resource use is observational, not a production optimization claim.
